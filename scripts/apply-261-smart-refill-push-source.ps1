# Apply migrations/261_smart_refill_push_and_reservation_source.sql (local Postgres 18).
...
$mig = Join-Path $root 'migrations\261_smart_refill_push_and_reservation_source.sql'
...
try {
    ...
    Write-Host '>> Apply 261_smart_refill_push_and_reservation_source.sql' -ForegroundColor Yellow
    ...
    & $psql -U postgres -h 127.0.0.1 -d kitplatform -c "INSERT INTO kit_schema_migrations (filename, applied_at) VALUES ('261_smart_refill_push_and_reservation_source.sql', NOW()) ON CONFLICT DO NOTHING;" 2>$null

    Write-Host '>> Verify' -ForegroundColor Yellow
    & $psql -U postgres -h 127.0.0.1 -d kitplatform -c "SELECT column_name FROM information_schema.columns WHERE table_name='customer_reservations' AND column_name='source_repurchase_suggestion_id';"
    Write-Host 'MIGRATION_261_OK' -ForegroundColor Green
}
finally {
    if (Test-Path $bak) {
