# Apply migrations/263_growth_care_actions.sql (local Postgres 18).
...
$mig = Join-Path $root 'migrations\263_growth_care_actions.sql'
...
try {
    ...
    Write-Host '>> Apply 263_growth_care_actions.sql' -ForegroundColor Yellow
    ...
    & $psql -U postgres -h 127.0.0.1 -d kitplatform -c "INSERT INTO kit_schema_migrations (filename, applied_at) VALUES ('263_growth_care_actions.sql', NOW()) ON CONFLICT DO NOTHING;" 2>$null
    Write-Host 'MIGRATION_263_OK' -ForegroundColor Green
}
finally {
    if (Test-Path $bak) {
