...
/** Chế độ điều chỉnh dòng trên cột CK — không ghi đè cột đơn giá mặc định. */
...
export function applyLineAdjustMode(line: CartLine, mode: PosLineAdjust | undefined): CartLine {
  ...
  if (mode === POS_LINE_ADJUST.UnitPrice) {
    return {
      ...
      discountType: undefined,
      discountValue: undefined,
      // giữ unitPrice hiện tại (đã có thể là catalog) — user nhập tiếp trong ô giá trị
    };
  }
...
export function applyLineAdjustValue(line: CartLine, value: number): CartLine {
  ...
  if (mode === POS_LINE_ADJUST.UnitPrice) {
    return { ...line, lineAdjust: POS_LINE_ADJUST.UnitPrice, unitPrice: value, discountType: undefined, discountValue: undefined };
  }
