...
internal sealed class ImageGenerationContractRepository
{
    private readonly IDbConnectionFactory _db;
    public ImageGenerationContractRepository(IDbConnectionFactory db) => _db = db;
    ...
    public sealed class Row
    {
        ...
        public string DocumentId { get; set; } = ImageGenerationContractRules.DocumentId;
    ...
    private const string SelectSql = """
        SELECT id AS Id, shot_id AS ShotId, shot_contract_id AS ShotContractId, prompt_id AS PromptId,
               ...
               prompt_sha256 AS PromptSha256, created_at AS CreatedAt, created_by AS CreatedBy,
        ...
        FROM pack_content.video_image_generation_contract
        """;
    ...
    public async Task<Row?> GetLatestApprovedAsync(Guid shotId, CancellationToken ct)
    {
        await using var conn = await _db.CreateOpenConnectionAsync(ct);
        return await conn.QuerySingleOrDefaultAsync<Row>(
            SelectSql + " WHERE shot_id = @ShotId AND contract_status = 'DIRECTOR_APPROVED' ORDER BY created_at DESC LIMIT 1;",
            new { ShotId = shotId });
    }
    ...
    {
        const string sql = """
            INSERT INTO pack_content.video_image_generation_contract (
                id, shot_id, shot_contract_id, prompt_id, series_id, character_id, era_id,
                ...
                shot_contract_sha256, prompt_sha256, created_by, updated_by, validated_at, validated_by
            ) VALUES (
                @Id, @ShotId, @ShotContractId, @PromptId, @SeriesId, @CharacterId, @EraId,
                ...
                @ShotContractSha256, @PromptSha256, @CreatedBy, @UpdatedBy, @ValidatedAt, @ValidatedBy
            );
            """;
        await using var conn = await _db.CreateOpenConnectionAsync(ct);
        ...
        {
            throw new ImageGenerationContractException("IMAGE_GENERATION_CONTRACT_INVALID", "Duplicate contract version.");
        }
    ...
    public async Task<Row> UpdateDraftAsync(Row row, CancellationToken ct)
    {
        ...
        var n = await conn.ExecuteAsync("""
            UPDATE pack_content.video_image_generation_contract
            SET payload_json = @PayloadJson::jsonb, canonical_json = @CanonicalJson, contract_sha256 = @ContractSha256,
                contract_status = @ContractStatus, shot_contract_id = @ShotContractId, prompt_id = @PromptId,
                ...
                prompt_sha256 = @PromptSha256, updated_at = NOW(), updated_by = @UpdatedBy,
    ...
    public async Task ApproveAsync(Guid id, string actor, CancellationToken ct)
    {
        ...
        if (n == 0)
            throw new ImageGenerationContractException("IMAGE_GENERATION_CONTRACT_LOCKED", "IMAGE_GENERATION_CONTRACT_LOCKED: không approve được.");
    }
    ...
    public async Task InsertEventAsync(Guid? contractId, Guid shotId, string eventType, object payload, string actor, CancellationToken ct)
    {
        await using var conn = await _db.CreateOpenConnectionAsync(ct);
        await conn.ExecuteAsync("""
            INSERT INTO pack_content.video_image_generation_contract_event
